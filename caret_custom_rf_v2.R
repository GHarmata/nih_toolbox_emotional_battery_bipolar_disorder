rf_v2 <- list(label = "Random Forest v2",
                  ...
                  # },
                  varImp = function(object, ...){
                    varImp <- randomForestExplainer::measure_importance(object, ...)
                    row.names(varImp) <- varImp$variable
                    out <- as.data.frame(varImp[,2:ncol(varImp)])
                    out
                  },
